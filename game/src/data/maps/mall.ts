...
registerMap({
  ...
  objects: [
    ...
    PR('mall_escalator_sign', 20, 7),
    PR('mall_floor_guide', 13, 11),
    PR('mall_info_counter', 4, 12),
    PR('mall_balloon', 15, 2),
    PR('mall_autodoor', 10, 14),
    PR('mall_bag_scrap', 13, 13),
    // a year of closure: the rubber plant left to dry, the sale board blown flat
    PR('mall_dead_leaves', 20, 9),
    PR('mall_dead_plant', 20, 9, { v: 0 }),
    PR('mall_fallen_sign', 15, 10),
    ...
    O('obj_gacha_corner', 16, 3, { w: 5, h: 2 }),
    O('obj_tanabata', 2, 5),
    ...
    O('obj_dead_plant', 20, 9),
    O('obj_fallen_sign', 15, 10, { w: 2, flat: true }),
...
registerMap({
  ...
  objects: [
    ...
    { t: 'sym', id: 'sym_mall_food_01', enemies: ['enemy_soujirou'], x: 6, y: 7, dir: 'right', move: 'soujirou', restoreAt: [1, 11], restoreOff: [5, 0] },
  ],
});
...
registerMap({
  ...
  objects: [
    ...
    // sits the party down, then runs evt_save_bench (lv_logic: lv_rest_bench)
    O('obj_rest_bench', 10, 5, { w: 2, script: 'lv_rest_bench' }),
    O('obj_mannequin', 15, 2),
    O('obj_dead_palm', 20, 6),
