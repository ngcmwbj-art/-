...
export class PaChain {
  ...
  constructor(private ctx: BaseAudioContext, dest: AudioNode, reverbLen = 1.8, lp = 3800) {
    ...
    rev.normalize = false;
    rev.buffer = makeIR(ctx, reverbLen, 3.0, 23, 0.6);
...
export function setSpaceOn(g: Graph, id: SpaceId, xfade = 0.3): void {
  ...
  if (!sp) return;
  g.space = id;
  g.fxRev.set(makeIR(g.ctx, sp.len, sp.decay, 5 + id.length), sp.hp, xfade);
...
/** Voices created while a capture list is set are appended to it. */
let capture: VoiceHandle[] | null = null;
...
export function voice(o: VoiceOpts): VoiceHandle {
  ...
  const g = cur();
  const c = g.ctx;
  const t0 = Math.max(o.at ?? c.currentTime, c.currentTime);
  ...
  if (wave === 'noise') {
    ...
    if (o.scoop) {
      osc.detune.setValueAtTime((o.detune ?? 0) + o.scoop, t0);
      osc.detune.linearRampToValueAtTime(o.detune ?? 0, t0 + (o.scoopTime ?? 0.04));
    }
    freqParam = osc.frequency;
    detParam = osc.detune;
    if (o.detuneSrc) {
      o.detuneSrc.connect(osc.detune);
      paramLinks.push([o.detuneSrc, osc.detune]);
    }
    if (o.detuneSrc2) {
      o.detuneSrc2.connect(osc.detune);
    ...
    const addFm = (fm: FmOpts) => {
      ...
      if (o.detuneSrc && !fm.fixed) {
        o.detuneSrc.connect(mod.detune);
        paramLinks.push([o.detuneSrc, mod.detune]);
      }
      if (o.detuneSrc2 && !fm.fixed) {
        o.detuneSrc2.connect(mod.detune);
        paramLinks.push([o.detuneSrc2, mod.detune]);
      }
  ...
  const dest = o.dest ?? busNode(g, o.bus);
  out.connect(dest);
  if (o.reverb) {
