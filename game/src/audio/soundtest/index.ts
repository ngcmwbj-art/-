...
function rowsFor(tab: TabId): Row[] {
  switch (tab) {
    case 'bgm': {
      const ids = [...BGM_ORDER.filter((id) => songTable.has(id)), ...[...songTable.keys()].filter((id) => !BGM_ORDER.includes(id))];
    ...
    case 'se': {
      ...
      for (const [id, info] of sfxInfo) {
        if (info.group !== g) {
          g = info.group;
          out.push({ kind: 'head', id: `#${g}`, label: g });
        }
    ...
    case 'amb':
      return AMBIENCE_IDS.map((id) => ({ kind: 'item', id, label: AMB_LABEL[id] ?? id }));
    case 'voice':
      return Object.keys(VOICES)
        .filter((id) => id !== 'sys')
        .map((id) => ({ kind: 'item', id, label: VOICES[id].label }));
    case 'cue':
      return CUES.map((c) => ({ kind: 'item', id: c.id, label: c.label, group: c.ref }));
  }
}
...
const KNOBS: Knob[] = [
  ...
  { name: 'SPACE', values: SPACE_IDS.length, get: () => Math.max(0, SPACE_IDS.indexOf(A.currentSpace())), set: (v) => A.setSpace(SPACE_IDS[v]), show: (v) => SPACE_IDS[v] },
...
class SoundTestScene implements Scene {
  private tab = 0;
  private sel: number[] = TABS.map(() => 0);
  private top: number[] = TABS.map(() => 0);
  private rows: Row[][] = TABS.map((t) => rowsFor(t.id));
  ...
  private updateList(inp: typeof game.input): void {
    ...
    if (s < this.top[this.tab] + 1) this.top[this.tab] = Math.max(0, s - 1);
    if (s > this.top[this.tab] + ROWS - 1) this.top[this.tab] = s - ROWS + 1;
    if (rows[this.sel[this.tab]]?.kind === 'head') this.sel[this.tab]++;
    if (inp.pressed('confirm')) this.activate(rows[this.sel[this.tab]]);
  }
  ...
  private activate(row: Row | undefined): void {
    ...
    switch (tab) {
      case 'bgm':
        if (A.currentBgmId() === row.id && !/jingle/.test(row.id)) A.stopBgm(0.5);
        else {
          const space = SONG_SPACE[row.id];
          if (space) A.setSpace(space);
          A.playBgm(row.id, { fade: 0.2 });
        }
      ...
      case 'amb':
        ...
        else A.playAmbient(row.id, { fade: 0.4 });
        this.last = { tab, id: row.id };
        break;
      case 'voice':
        this.say(row.id, VOICE_SAMPLES[row.id] ?? VOICE_SAMPLES.default);
        this.last = { tab, id: row.id };
        break;
      case 'cue': {
        const cue = CUES.find((c) => c.id === row.id);
  ...
  private drawList(g: Gfx): void {
    ...
    // header: section title with a vermilion pencil underline, and the count
    const title = { bgm: '曲と ジングル', se: '効果音', amb: '環境音（ループ）', voice: '文字送りの声', cue: '演出（キューシート）' }[tab.id];
    g.text(title, TEXT_X, 23, { color: C.ink });
    const tw = textW(title);
    for (let x = 0; x < tw + 4; x++) g.px(TEXT_X - 1 + x, 40 + (x % 7 === 3 ? 1 : 0), C.shu);
  ...
  private drawBarMap(g: Gfx, y: number): void {
    ...
    let loop = def.loop;
    let off = 0;
    // the boss list holds both phases: show the one playing
    if (def.id === 'bgm_boss' && loop.length === 48) {
  ...
  private drawKnobCard(g: Gfx): void {
    const ctx = g.ctx;
    this.drawCardFrame(g, KNOB_Y, KNOB_H, 'つまみ');
    const y0 = KNOB_Y + 10;
    KNOBS.forEach((k, i) => {
      const y = y0 + i * 9;
      ...
      if (sel)
        g.alpha(0.75, () => {
          g.rect(CX - 2, y - 1, CW + 2, 9, C.marker);
        });
      ...
      if (k.values <= 4) {
        ...
        g.rect(vx, y + 2, Math.round((v / 10) * w), 3, C.shu);
        f5(ctx, String(v), vx + w + 6, y, C.ink);
      }
    });
  }
